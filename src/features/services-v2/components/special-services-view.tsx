...
import { specialServices } from "../data/services-v2-data";
import type { SpecialService } from "../data/services-v2-data";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

type SpecialServicesViewProps = {
  onSelectService: (service: SpecialService) => void;
  onBack: () => void;
};

export function SpecialServicesView({
  onSelectService,
  onBack,
}: SpecialServicesViewProps) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.from("[data-ss-header]", { autoAlpha: 0, y: 30, duration: 0.7 })
        .from(
          "[data-ss-sub]",
    ...
    <div ref={sectionRef}>
      ...
      <div className="flex flex-col pt-24 pb-10 md:pt-24 lg:pt-28 lg:pb-12">
        <GeneralWrapper>
          ...
          >
            From window cleaning to post-construction, our specialist team
            handles niche requirements with the same exacting standard.
          </p>

          {/* Search */}
          <div data-ss-search className="mt-6 w-full max-w-md">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search specialist services…"
      ...
      <section className="pb-16 pt-2">
        <GeneralWrapper>
          {filtered.length === 0 ? (
            ...
            <div data-ss-list className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((service) => (
                <article
                  key={service.id}
                  data-ss-card
                  className="group relative flex flex-col overflow-hidden rounded-xl border border-primary/5 bg-white shadow-[0_8px_28px_rgba(8,10,88,0.06)] transition-shadow duration-300 hover:shadow-[0_16px_44px_rgba(8,10,88,0.1)]"
                >
                  {/* Image */}
                  <div className="relative h-44 w-full overflow-hidden">
                    ...
                    <div className="absolute left-4 top-4">
                      <span className="inline-flex items-center rounded-full bg-secondary-container px-3 py-0.5 font-manrope text-[10px] font-extrabold uppercase tracking-widest text-on-secondary-container">
                        Specialist
                      </span>
                    </div>
                  </div>
